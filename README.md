...
Для запуска необходимо установить зависимости и текст:

- [`data/book.txt`](data/book.txt) - книга без форматирования
- [`data/book_en.txt`](data/book_en.txt) - только текст книги
- [`data/book_ru.txt`](data/book_ru.txt) - перевод текста книги на русском языке
  (при помощи google translate)

Перед запуском рекомендуется создать и активировать виртуальное окружение
([инструкция](https://docs.python.org/3/library/venv.html)).
...
Для корректной работы алгоритмов из 5 практической работы необходимо 2 текста:
один на английском языке, другой на русском - перевод. Их нужно разместить в
[`data/book_en.txt`](data/book_en.txt) - текст на английском,
[`data/book_ru.txt`](data/book_ru.txt) - перевод на русском языке.

##### Запуск программы ([5.ipynb](5.ipynb))

Обучены 2 модели [`lstm_model.keras`](lstm_model.keras) и
[`s2s_model.keras`](s2s_model.keras). Если нет желания обучать модели, то нужно
закомментировать код обучения (рядом с `Обучение LSTM` и `Обучение модели`)

```
jupyter notebook 5.ipynb
```

По итогу практической работы была построена модель для генерации осмысленного
текста на основе книги. Текст сложно назвать связным, нужно больше эпох для
обучения модели, возможно, это поможет. Но у меня толкового результата не
получилось...\
Также при помощи метода sequence-to-sequence была построена модель для перевода
текста с английского на русский. Здесь вообще никакого результата не удалось
достигнуть (только плохих результатов). Делал с разными данными (но на основе
книги), с разной архитектурой, с разными параметрами, все равно не получилось
добиться даже среднего результата. Это точно связано с текстом, потому что 1.
там много длинных предложений, что плохо; 2. сделан перевод всего текста, а не
по предложениям из-за чего где-то **точно** происходит сдвиг из-за которого все
ломается. Скорее всего, для реализации этого метода книги плохо подходят.
...
Так как в работе необязательно использовать результаты из предыдущих работ, было
принято решение попробовать PyTorch и другой датасет. Стоит отметить, что для
запуска всего jupyter блокнота необходимо следовать всем этапам из предыдущих
инструкций. В работе используется датасет с расшифровкой из сериала "Офис".
Данный набор данных расположен в
[`data/The-Office-Lines-V4.csv`](data/The-Office-Lines-V4.csv) (он загружен из
[kaggle](https://www.kaggle.com/datasets/nasirkhalid24/the-office-us-complete-dialoguetranscript/data)).

#### Инструкция

Для запуска необходимо установить зависимости и текст:

- [`data/The-Office-Lines-V4.csv`](data/The-Office-Lines-V4.csv) - csv файл с
  диалогами (колонка `line`)
...
```
pip install -U notebook spacy nltk matplotlib seaborn scikit-learn gensim tensorflow torch pandas
```

##### Запуск программы ([6.ipynb](6.ipynb))

Была обучена модель [`chatbot_model`](chatbot_model.pth). Если нет желания
обучать модель, то нужно закомментировать код обучения (раздел
`Обучение чат-бота`).
...
Для простоты использования был написан python скрипт [`6.py`](6.py)
(по-умолчанию, скрипт запускается с обученной моделью
([`chatbot_model`](chatbot_model.pth)), для обучения модели необходимо запустить
скрипт с флагом `--train` и `--epochs n` (где n - количество эпох)
(опционально)).
...
По итогу практической работы была разработана диалоговая программная система
(чат-бот). Даже получилось получить какой-то осмысленный ответ от бота. Данная
система показала результат намного лучше, чем генератор на основе шаблонов. Я
предполагаю, что если его обучить на всем наборе данных и увеличить количество
эпох, результат будет еще лучше.
